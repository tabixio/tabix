...
((angular, smi2,  $) => {
    ...
    function OverviewController($scope,
        ...
        $scope.init = () => {
            ...
            $scope.widgets=[
                {id: 0, x: 0, y: 0, sizeX: 8, sizeY: 2},
                {id: 1, x: 0, y: 2, sizeX: 8, sizeY: 2},
                {id: 2, x: 0, y: 4, sizeX: 8, sizeY: 2},
            ...
            API.fetchQuery(`SELECT * FROM system.clusters`).then(function ( queryResult ) {
                $scope.setWidgets(1,new WidgetTable(new DataProvider(queryResult)));
            });


            API.fetchQuery(` SELECT metric,toInt64(value) as value,'metrics' as type FROM system.metrics ORDER BY metric `).then(function ( queryResult ) {
                $scope.setWidgets(2,new WidgetTable(new DataProvider(queryResult)));
            });
