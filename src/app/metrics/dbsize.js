...
((angular, smi2,  $) => {
    'use strict';
    angular.module(smi2.app.name).controller('DBSizeController', DBSizeController);
    DBSizeController.$inject = [
        '$scope',
        'API',
        'ThemeService'
    ];

    function DBSizeController($scope,
                              API,
                              ThemeService) {
        ...
        $scope.init = () => {
            ...
            console.info("Init DBSizeController");
            //
            ...
            API.fetchQuery(`SELECT database,table,name, data_compressed_bytes, data_uncompressed_bytes FROM system.columns`).then(function ( queryResult ) {
                ...
                $scope.setWidgets(0,new WidgetDraw(new DataProvider(queryResult),drawCommand ));
